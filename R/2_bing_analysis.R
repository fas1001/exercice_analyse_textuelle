...
# Étape 5: Application de l'analyse de sentiment BING
# BING est un lexique qui classifie les mots comme "positif" ou "négatif"
bing <- get_sentiments("bing")    # Chargement du lexique BING
sentiment_scores <- inner_join(
  tokens_clean,    # Table principale contenant les mots nettoyés
  bing,           # Table de référence contenant les classifications de sentiment
  by = "word"      # Jointure sur la colonne "word"
)                  # inner_join garde uniquement les mots qui ont une classification dans le lexique BING

# Étape 6: Calcul du sentiment par phrase
# Pour BING, nous allons convertir les classifications textuelles en valeurs numériques
sentence_sentiment <- sentiment_scores %>%
  # Convertir les catégories "positive" et "negative" en valeurs numériques
  ...
  summarise(
    # Compter les mots positifs dans la phrase
    positive_words = sum(sentiment == "positive", na.rm = TRUE),
    # Compter les mots négatifs dans la phrase
    negative_words = sum(sentiment == "negative", na.rm = TRUE),
    # Calculer le sentiment net (positif - négatif)
    sentence_sentiment = sum(sentiment_value, na.rm = TRUE),
...
article_sentiment <- sentence_sentiment %>%
  ...
  summarise(
    ...
    # Somme des mots positifs dans toutes les phrases concernant les réfugiés
    total_positive_words = sum(positive_words, na.rm = TRUE),
    # Somme des mots négatifs dans toutes les phrases concernant les réfugiés
    total_negative_words = sum(negative_words, na.rm = TRUE),
    # Ratio positif/négatif (avec une petite valeur pour éviter la division par zéro)
    sentiment_ratio = total_positive_words / (total_negative_words + 0.0001),
...
# Utilisation de final_df de notre analyse de sentiment BING
...
p <- ggplot(df_plot, aes(x = country, y = mean_sentiment, fill = country)) + 
  ...
  # Étiquettes des axes comme dans votre exemple
  labs(
    x = "Pays", 
    y = "Score moyen de sentiment",
    title = "Sentiment moyen envers les réfugiés par pays (BING)",
